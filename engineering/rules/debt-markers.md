...
**The hook cannot work out on its own which lines are new.** It is handed paths. A path names a file, a file has lines, and nothing in either says which of them this change wrote. So the change is a second, explicit input rather than something inferred: `hooks/check-structure.py --changed <spec>` hands the spec to a pinned `git diff` and decides the condition over the lines that diff adds. `--changed --cached` is the staged diff, which is what a pre-commit hook has; `--changed origin/main...HEAD` is a pull request's own change. It is one argument in the same command line either way, so the local run and the CI run are the same run rather than two gates that can disagree.

**The spec is a revision or a range, and an option in its place is refused.** The only options it takes are `--cached` and `--staged`. The spec lands on the same command line as the diff's own options and git lets the last flag win, so `--name-only`, `--quiet` or `--output=` in that position would empty the scope, and `-U5` would widen it past the lines the change wrote — each of them while the run still exited 0. The diff itself is pinned for the same reason: colour, a replacement diff driver, a content filter and a dropped path prefix are ordinary git configurations, and an unpinned invocation scopes the condition to nothing, or to the wrong file, on a machine that has any of them set.
...
**The scoping is the caller's, and the hook says so rather than guessing.** Run without `--changed`, the condition reads the tree and reports every unreferenced marker in it, which is the retrospective answer to a prospective question. Nothing in the hook can close that gap, because nothing it is handed carries the change. What it can do, and does, is refuse to guess: `--changed` with no revision, with an option in place of one, in a directory git cannot answer for, over a path outside that repository, or during a merge git will only describe as unresolved, exits non-zero with the reason instead of reporting a clean tree. A gate that silently degrades to seeing nothing is the one failure mode worse than a noisy one.

**A file git does not report as changed is out of scope, and an untracked file is one.** Under `--changed <rev>` a file that has never been added is invisible, because a diff against a revision has nothing to compare it to; under `--changed --cached` it is in scope as soon as it is staged, which is where a pre-commit hook meets it. That is a property of the input and not a judgment, and it is the reason the staged form is the one a local hook should use.
