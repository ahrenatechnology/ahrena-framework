...
**It does not cover DRY, and DRY is where the next argument will be.** Duplication and abstraction are not the same question, the rule of three is a different predicate from this trigger, and collapsing them is how "extract the common part" becomes a justification for a base class nobody wanted. That belongs in the artifact that covers DRY, and no such artifact exists in this plugin yet.
