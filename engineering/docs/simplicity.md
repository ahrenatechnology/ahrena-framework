...
**It does not say how to get from the shape in the tree to the simpler one.** Everything above decides which shape is right: whether an abstraction is justified, which SOLID conditions the trigger gates, where the KISS thresholds came from. None of it is a procedure, and the move it implies — reshaping working code without changing what a consumer observes — has failure modes of its own that no position on simplicity prevents. Bounding the behaviour, getting a test that fails when it moves, keeping each transformation reversible, and checking at the end that the smell is smaller rather than relocated are steps, and [`skills/refactoring-safely/SKILL.md`](../skills/refactoring-safely/SKILL.md) carries them. That last check is the one this document is behind: a transformation that satisfies every threshold here and merely moves the complexity behind a new name has paid the cost the abstraction trigger exists to refuse, and no detector reports it.
