...
**It does not cover the rest of the tactical set.** The behaviour a boundary leaves over is [`rules/domain-services.md`](../rules/domain-services.md), and [`docs/domain-services.md`](./domain-services.md) is its companion; specification and factory were examined with it, earned no rule, and are entries in [`docs/patterns.md`](./patterns.md) with the measurements that refused them recorded beside the service. The modelling half of value object still has no artifact. The mechanical half is `rules/value-semantics.md` and the entity contract is `rules/domain-model.md`; what remains is absent because this plugin has nothing useful to say about when to reach for it yet, which is the same scope `docs/patterns.md` claims for its own entries.
