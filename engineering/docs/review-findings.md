...
Three conditions in this plugin are decided by running something: condition 3 of `rules/solid.md` runs the supertype's tests against a subtype, condition 4 of `rules/cross-cutting-concerns.md` runs a replay test, and condition 3 of `rules/contract-first.md` runs the contract tests. A reviewer that wants those answers has to build the project and run its suite.
...
So the procedure does not do it. It records an `unchecked` finding that names the three conditions it could not decide and the reason, and it continues with everything that only reads: the shipped detector, the judgment conditions, and the contract comparison, which is a diff of two documents and executes nothing.
