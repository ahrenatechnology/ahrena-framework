...
A header line's list marker is `-`, `*` or `+`, as an alternatives item is; `-` is what the template writes and nothing turns on the choice.

## Conditions

Each of these is decided by `hooks/check-decision-records.py`.

1. **Every file in the decision directory, and in every directory under it, is named `ADR-` plus three digits, a hyphen, and a kebab-case slug**, matching `ADR-\d{3}-[a-z0-9]+(-[a-z0-9]+)*\.md`. Three digits rather than a variable width so the directory sorts in numeric order in every tool that lists it. A file that is not a record does not belong in a directory whose whole contents are addressable by number.

    Subdirectories are read because a record in one is still a record. If they were not, `git mv docs/adr/ADR-004-*.md docs/adr/archive/` would take that record's number, its supersession and the gap it leaves out of the gate's sight — a deletion that condition 2 never sees, which is the one thing condition 2 exists to make impossible.

    Two names are not records and do not fail: `README.md`, which `adr-tools` writes as the log's index, and `.gitkeep`, which is the only way to commit the directory before the first record exists. Failing those would mean an empty log cannot be committed and the ecosystem's own tool cannot be run, which buys nothing.

2. **The numbers are unique and run from 001 with no gaps.** Two conditions in one state, because both failures are the same read. A duplicate is what two authors produce concurrently, and it silently breaks every reference by number. A gap is what a deleted record leaves, and a decision that was made and then erased is the one case a log exists to prevent. `ADR-000` fails: 001 is where the numbering starts, and a zeroth record is a filename written wrong. The predecessor stated this as "sequential numbering is inviolable" and enforced it with nothing.

    A log whose lowest number is far from 001 is one record numbered wrong and not a hundred deletions, so past ten missing numbers the gap is stated once rather than once per number.

3. **The first line is `# ADR-nnn: ` followed by a non-empty title, and `nnn` matches the filename.** The first line the file has, ignoring a leading byte-order mark and any blank line above it; anything else before the heading fails. A record is almost always written by copying the last one, and the heading is what gets forgotten. This is the same condition `foundation/rules/naming.md` states as identity — the name inside matches the name on disk — applied to the one place a record carries its own number twice.

    `nnn` is three digits here as it is in the filename. `# ADR-7:` and `# ADR-0007:` fail on their width and say so, rather than reading as a record with no heading at all.

4. **A `- **Status:**` line is present and its value is `accepted`, `superseded` or `withdrawn`.** The field name and the value are both read without regard to case, as condition 7 reads a heading. The enum is closed at three. `proposed` and `rejected` were dropped and `deprecated` was renamed; `docs/decision-records.md` gives the reason for each.
...
6. **Supersession resolves in both directions, and the chain ends.** A record whose status is `superseded` carries a `- **Superseded by:**` line naming a record that exists, and that record carries a `- **Supersedes:**` line naming this one. The converse holds too: a `Supersedes` line names a record that exists, whose status is `superseded`, and whose `Superseded by` points back. Every other arrangement fails.

    **No record names itself and no chain closes into a ring.** Every edge of a two-cycle resolves in both directions, and so does every edge of a ring of three; what they produce is a log in which every record reads as replaced and none is current, which is the exact state the condition exists to prevent. The chain is followed, not only each link.

    **A record may supersede more than one.** Condition 2 forbids deleting a record, so merging two decisions into one is a supersession with two edges: the new record carries a `Supersedes` line for each, or names both on one line separated by a comma, and each of them carries its own `Superseded by` pointing at it. The reverse is not allowed — a record has one successor, because two would leave no answer to which decision replaced it.

    **A single-valued header field written twice fails.** `Status`, `Date` and `Superseded by` are one line each, and each repeat is reported under the condition that owns the field rather than here. Reading the last quietly would let `Status: banana` followed by `Status: accepted` pass, and would leave the reader no way to know which line the gate read.

7. **The four sections are present as `##` headings — `Context`, `Decision`, `Consequences`, `Alternatives considered` — and the alternatives section carries at least one list item.** A list item is a bullet (`-`, `*`, `+`) or an ordered one (`1.`, `1)`); Markdown has two kinds and the condition asks for a list item. A section runs until the next `##` or `#` heading, so an item under a later heading is not under this one.

    Heading matching ignores case and is done outside fenced code, so a record quoting a heading is not one. A fence that is never closed is reported as itself: hiding the rest of the file behind it would report three sections missing that are plainly present.
...
**Only the default path is allowed to be absent.** A missing `docs/adr/` is a project with no decision log and it passes. A path given on the command line and not found is a typo or a step wired to the wrong directory, and it fails — the two states are indistinguishable from the output otherwise, and a gate that is green because it is looking at nothing is worse than no gate.
