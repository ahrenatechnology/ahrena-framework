...
**Trunk, release branches and a detached checkout are outside.** `main`, `master` and anything under `release/` are not working branches, and `HEAD` is what git answers with when no branch is checked out. The hook prints that it has nothing to decide and moves on. Whether trunk may be committed to directly is `protected-trunk.md`'s question, not this one's.
