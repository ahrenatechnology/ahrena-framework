...
3. **The subject is at most 72 characters.** `git log` indents every line of a message by four spaces, so 72 is the longest subject that still fits a default 80-column terminal without wrapping. That arithmetic is the threshold's whole provenance; it arrives here by way of the predecessor framework's `codex-commit-standards`, which stated 72 without deriving it. It was **not** adjusted to fit this repository: 6 of the 38 subjects on trunk are over it, the longest reaching 88. A measured threshold is now available where none was before, and it is declined rather than absent. The body of the distribution ends at 76 and the next two subjects sit at 83 and 88, leaving 77 to 82 empty — a six-character span, and exactly the shape [the engineering plugin's clean-code doc](../../engineering/docs/clean-code.md) asks for when it places a threshold at the bottom of a gap rather than at the top of the data. Applied here that method returns 77. It is refused because the span was opened by two commits rather than by a change of kind, because 77 fails two subjects where 72 fails six and so loosens the rule to admit what was written last, and because 77 has no derivation while 72 has the terminal arithmetic above. `docs/contribution-flow.md` carries the distribution and the three refusals in full.

4. **The subject does not end in a period.** A subject is a title, and a title that ends in a full stop spends a character of a budget condition 3 already found tight. Measured rather than inherited: 0 of 38 subjects on trunk end in one, so this condition describes the practice exactly and costs nothing to keep.

5. **A body is separated from the subject by one blank line.** The state is a second line that is not empty when a third line exists. Every tool that splits a message into subject and body splits on that blank line, so a message without it has one field where it should have two. Measured: 37 of the 38 commits carry a body and all 37 comply.
...
**Merge commits are skipped rather than judged.** The state is a commit object with more than one `parent` header; its message was written by whatever performed the merge and it carries no change of its own to describe. The exemption has since fired twice. Two pull requests merged trunk into their own branches to resolve a conflict, and the hook reported one skipped merge commit on each; the remote carries none today only because both branches were deleted after merging. So it guards a shape that occurs, produced by resolving a conflict on a branch as much as by the forge, and it is no longer a carve-out for something unobserved.
